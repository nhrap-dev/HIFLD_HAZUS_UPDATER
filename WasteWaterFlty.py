...
# Proof of Concept - HIFLD WasteWaterFlty into HAZUS WasteWaterFlty.
...
url = cfgParser.get("HIFLD OPEN DATA URLS", "WasteWaterFlty_CSV_URL")
...
# Download CSV
try:
    tempCSVPath = os.path.join(tempDir, "WasteWaterFlty.csv")
...
try:
    for state in existingDatabaseList:
        ...
        cursor = conn.cursor()
        try:
            if cursor.tables(table="hifld_WasteWaterFlty", tableType="TABLE").fetchone():
                print " hifld table already exists, dropping table..."
                cursor.execute("DROP TABLE hifld_WasteWaterFlty")
                conn.commit()
                print " done"
            else:
                print " hifld table doesn't exist"
            print " Creating hifld table..."
            createTable = "CREATE TABLE hifld_WasteWaterFlty \
                            (SHAPE geometry, \
                            WasteWaterFltyID varchar(8), \
            ...
            for row in reader:
                if row["CWP_STATE"] == state:
                    ...
                    try:
                        ...
                        print " cursor execute insertData CSV exception(CWP_Name={}): {}".format(row["CWP_Name"], (e))
        except Exception as e:
...
try:
    for state in existingDatabaseList:
        ...
        conn = pyodbc.connect(connectString, autocommit=False)
        cursor = conn.cursor()
        hifldtable = "["+state+"]..[hifld_WasteWaterFlty]"
        ...
        try:
            cursor.execute("UPDATE "+hifldtable+" SET WasteWaterFltyId = '"+state+\
                           "' + RIGHT('000000'+cast(IDseq as varchar(6)),6)")
            conn.commit()
        except Exception as e:
            print " cursor execute UPDATE WasteWaterFltyId: {}".format((e))
...
##            print " cursor execute UPDATE PHONENUMBER: {}".format((e))
        ...
        try:
            ...
            # Update hifldtable...
            cursor.execute("UPDATE "+hifldtable+" \
                            SET [COSTKUSD] =  (143000 * " + RSMeansNonResAvg + ") ")
            conn.commit()
        except Exception as e:
...
try:
    for state in existingDatabaseList:
        print state
        hifldTable = "["+state+"]..[hifld_WasteWaterFlty]"
        hzTable = "["+state+"]..[hzWasteWaterFlty]"
        eqTable = "["+state+"]..[eqWasteWaterFlty]"
        flTable = "["+state+"]..[flWasteWaterFlty]"
        ...
        try:
            cursor.execute("INSERT INTO "+hzTable+" \
                            ...
                            \
                            FROM "+hifldTable+\
                            " WHERE WasteWaterFltyID IS NOT NULL \
                            AND CENSUSTRACTID IS NOT NULL \
                            ORDER BY WasteWaterFltyID ASC")
            conn.commit()
        except Exception as e:
