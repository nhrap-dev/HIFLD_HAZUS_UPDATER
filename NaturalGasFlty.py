...
# Proof of Concept - HIFLD NaturalGasFlty into HAZUS NaturalGasFlty.
...
url = cfgParser.get("HIFLD OPEN DATA URLS", "NaturalGasPlants_CSV_URL")
url2 = cfgParser.get("HIFLD OPEN DATA URLS", "NaturalGasCompressorStations_CSV_URL")
...
# Download CSV
try:
    tempCSVPath = os.path.join(tempDir, "NaturalGasPlants.csv")
...
# Download CSV2
try:
    tempCSVPath2 = os.path.join(tempDir, "NaturalGasCompressorStations.csv")
...
try:
    for state in existingDatabaseList:
        ...
        cursor = conn.cursor()
        try:
            if cursor.tables(table="hifld_NaturalGasFlty", tableType="TABLE").fetchone():
                print " hifld table already exists, dropping table..."
                cursor.execute("DROP TABLE hifld_NaturalGasFlty")
                conn.commit()
                print " done"
            else:
                print " hifld table doesn't exist"
            print " Creating hifld table..."
            createTable = "CREATE TABLE hifld_NaturalGasFlty \
                            (SHAPE geometry, \
                            NaturalGasFltyID varchar(8), \
        ...
        try:
            ...
            for row in reader:
                if row["STATE"] == state:
                    ...
                    sqlInsertData = "INSERT INTO ["+state+"]..[hifld_NaturalGasFlty] ("\
                                    +hifld_Columns+") \
                                    VALUES \
                                    (?,?,?,?,?,\
                                    ?,?,?,?)"
...
print "Copy Downloaded HIFLD CSV2 to SQL Staging Table..."
RowCountCSV2Dict = {}
try:
    # Define the columns that data will be inserted into
    hifld_Columns = "UtilFcltyCLASS, \
            ...
            for row in reader2:
                if row["STATE"] == state:
                    ...
                    try:
                        ...
                        print " cursor execute insertData CSV2 exception(NAME={}): {}".format(row["NAME"], (e))
        except Exception as e:
            print " csv2 dict exception: {}".format((e))
...
try:
    for state in existingDatabaseList:
        ...
        conn = pyodbc.connect(connectString, autocommit=False)
        cursor = conn.cursor()
        hifldtable = "["+state+"]..[hifld_NaturalGasFlty]"

        # NaturalGasFltyId (State abbreviation plus 6 digits eg WA123456,
        # this must be unique and will persist across four tables.
        # IDSeq should be unique, non null and int)
        try:
            cursor.execute("UPDATE "+hifldtable+" SET NaturalGasFltyId = '"+state+\
                           "' + RIGHT('000000'+cast(IDseq as varchar(6)),6)")
            conn.commit()
        except Exception as e:
            print " cursor execute UPDATE NaturalGasFltyId: {}".format((e))
...
##            print " cursor execute UPDATE PHONENUMBER: {}".format((e))
        ...
        try:
            ...
            # Update hifldtable...
            cursor.execute("UPDATE "+hifldtable+" \
                            SET [COSTKUSD] = (1750 * " + RSMeansNonResAvg + ") ")
            conn.commit()
        except Exception as e:
        ...
        try:
            ...
            print " cursor execute NameTRUNC to be under limit exception: {}".format((e))
        try:
            cursor.execute("UPDATE "+hifldtable\
                           +" SET AddressTRUNC = \
            ...
            print " cursor execute AddressTRUNC to be under limit exception: {}".format((e))
        try:
            cursor.execute("UPDATE "+hifldtable\
                           +" SET CityTRUNC = \
            ...
            print " cursor execute CityTRUNC to be under limit exception: {}".format((e))
        try:
            cursor.execute("UPDATE "+hifldtable+" \
                            SET [ZIPTRUNC]  = \
            ...
            print " cursor execute ZIPTRUNCto be under limit exception: {}".format((e))
        try:
            cursor.execute("UPDATE "+hifldtable\
                           +" SET OwnerTRUNC = \
            ...
            print " cursor execute OwnerTRUNC to be under limit exception: {}".format((e))
##            cursor.execute("UPDATE "+hifldtable\
##                           +" SET ContactTRUNC = \
...
try:
    for state in existingDatabaseList:
        print state
        hifldTable = "["+state+"]..[hifld_NaturalGasFlty]"
        hzTable = "["+state+"]..[hzNaturalGasFlty]"
        eqTable = "["+state+"]..[eqNaturalGasFlty]"
        flTable = "["+state+"]..[flNaturalGasFlty]"
        ...
        try:
            cursor.execute("INSERT INTO "+hzTable+" \
                            ...
                            \
                            FROM "+hifldTable+\
                            " WHERE NaturalGasFltyID IS NOT NULL \
                            AND CENSUSTRACTID IS NOT NULL \
                            ORDER BY NaturalGasFltyID ASC")
            conn.commit()
        except Exception as e:
