...
# Proof of Concept - HIFLD PortFlty into HAZUS PortFlty.
...
url = cfgParser.get("HIFLD OPEN DATA URLS", "PortFlty_CSV_URL")
...
# Download CSV
try:
    tempCSVPath = os.path.join(tempDir, "PortFlty.csv")
...
print "Create 'hifld_PortFlty' Staging Table..."
try:
    for state in existingDatabaseList:
        ...
        cursor = conn.cursor()
        try:
            if cursor.tables(table="hifld_PortFlty", tableType="TABLE").fetchone():
                print " hifld_PortFlty already exists, dropping table..."
                cursor.execute("DROP TABLE hifld_PortFlty")
                conn.commit()
                print " done"
            else:
                print " hifld_BusFlty doesn't exist"
            print " Creating hifld_BusFlty table..."
            createTable = "CREATE TABLE hifld_PortFlty \
                            (PortFltyID varchar(8), \
...
try:
    ...
    hifld_PortFlty_Columns = "TranspFcltyCLASS, \
                                            NAME, \
                                            ADDRESS, \
                                            CITY, \
                                            STATE, \
    ...
    for state in existingDatabaseList:
        ...
        try:
            ...
            for row in reader:
                if row["state_post"] == state:
                    RowCountCSV1 += 1
                    # This list order must match the order of the created table that it's being inserted into                 
                    sqlInsertData = "INSERT INTO ["+state+"]..[hifld_PortFlty] ("\
                                    +hifld_PortFlty_Columns+") \
                                    VALUES \
                                    (?, ?, ?, ?, ?,\
                                    ?, ?, ?, ?, ?)"
                    ...
                    if len(row["zipcode"]) > 0:
                        try:
                            ...
                            Zip = str(row["zipcode"])[0:5]
                        except Exception as e:
                            Zip = ""
                            print " zip parse exception(objectid={}): {}".format(row["objectid"], (e))
                    else:
                        Zip = ""
                    try:
...
print "Calculate hifld_PortFlty fields..."
try:
    for state in existingDatabaseList:
        print state
        connectString = "Driver={SQL Server};Server="+userDefinedServer+";\
        ...
        conn = pyodbc.connect(connectString, autocommit=False)
        cursor = conn.cursor()
        hifldtable = "["+state+"]..[hifld_PortFlty]"
        ...
        try:
            cursor.execute("UPDATE "+hifldtable+" SET PortFltyId = '"+state+\
        ...
        try:
            ...
            # Update hifldtable...
            cursor.execute("UPDATE "+hifldtable+" \
                            SET [COSTKUSD] = (3160 * " + RSMeansNonResAvg + ")")
            conn.commit()
        except Exception as e:
...
try:
    for state in existingDatabaseList:
        print state
        hifldTable = "["+state+"]..[hifld_PortFlty]"
        hzTable = "["+state+"]..[hzPortFlty]"
        eqTable = "["+state+"]..[eqPortFlty]"
        ...
        try:
            cursor.execute("INSERT INTO "+hzTable+" \
                            ...
                            \
                            FROM "+hifldTable+\
                            " WHERE PortFltyID IS NOT NULL \
                            AND CensusTractId IS NOT NULL \
                            ORDER BY PortFltyID ASC")
            conn.commit()
        except Exception as e:
